...
#### Note 2: In the following, we will occasionally use terms user, admin and developer to refer to specific roles related to the lifecycle of control loops. User role is responsible for defining master custom resource (and other artifacts as custom policy templates or components as external applications if nescessary) to run control loop instances; it represents an owner of the managed object who wants this object to be controlled in a specific way. Administrator is responsible for preparation of the whole environment needed to run control loop sby users (this includes in particular setting up Kubernetes cluster where control loops are run, generating operator images, possibly also providing external applications and selected policy templates). And developer is responsible for writing the code of controllers that provide the functionality of loop components.
