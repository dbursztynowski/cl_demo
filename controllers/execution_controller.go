...
func (r *ExecutionReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	l.V(1).Info("Received Action from Decision: " + fmt.Sprintf("%v", Execution.Spec.Message) + " | timestamp: " + Execution.Spec.Time)
	function_result := gjson.Get(Execution.Spec.Config, "function.name")
	function_paramerer := gjson.Get(Execution.Spec.Config, "function.parameter")
	if function_result.Str == "Podpatch" && function_paramerer.Str == "cpu" {
		cpu := gjson.Get(Execution.Spec.Message, "cpu")
		mytools.Podpatch("default", "open5gs-upf-55cbbcc847-9kmkq", cpu.Str, ctx, r.Client, l, req)
		l.V(1).Info("Execution using kubectl API")
	} else {
