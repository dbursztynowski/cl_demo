...
func (r *MonitoringReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	url := "http://" + Monitoring.Spec.Source.Addresse
	...
	if err != nil {
		fmt.Println("Error on JSON convertion :", err)
		return ctrl.Result{}, err
	}
	...
	/*	ObjectData := -1
		...
		if len(data) > 0 {
			for key, value := range Monitoring.Spec.MonitoringPolicies.Data {
				...
				thresholdvalue, err := strconv.Atoi(Monitoring.Spec.MonitoringPolicies.TresholdValue[key+"-thresholdvalue"])
				l.V(2).Info(Monitoring.Spec.MonitoringPolicies.TresholdValue[key+"-thresholdvalue"])
				...
				switch Monitoring.Spec.MonitoringPolicies.TresholdKind[key+"-thresholdkind"] {
				case "inferior":
					l.V(2).Info("Inferior")
	...
	*/
	...
	l.V(1).Info("message Send to Decision " + fmt.Sprintf("%v", message))
	...
	return ctrl.Result{
		Requeue:      true,
		RequeueAfter: time.Duration(Monitoring.Spec.Source.Interval) * time.Second,
	}, nil
}
...
*/
func (r *MonitoringReconciler) ApplyDecision(ctx context.Context, monitoring *closedlooppoocv1.Monitoring, l mytools.VerbosityLog, Message string) error {
	...
	l.V(2).Info("Send Message to Decision", "Message", Message)
...
func (r *MonitoringReconciler) SetupWithManager(mgr ctrl.Manager) error {
	return ctrl.NewControllerManagedBy(mgr).
		For(&closedlooppoocv1.Monitoring{}).
		Complete(r)
}
